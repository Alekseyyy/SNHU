...
# A modified version of the "task.py" script that came with this challenge
# Modified by Aleksey/EpsilonCalculus/EntropyThot of The Rocking Philosophers
...
def decrypt():
  def bit_flip_fixed(x, w):
    flip_str = base64.b64decode(w)
    return xor32(flip_str, x)

  def main_procedure(w):
    alice = DiffieHellman(bit_flip_fixed(alice_seed, w))
    bob = DiffieHellman(os.urandom(16), alice.prime)
    ...
    return bob.my_number, base64.b64encode(iv).decode(), base64.b64encode(enc_flag).decode()

  a, b, c = main_procedure("test")
  print(a)
