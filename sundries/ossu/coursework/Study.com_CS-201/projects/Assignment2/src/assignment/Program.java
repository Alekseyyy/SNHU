 ...
 *   apply the Boyer-Moore search algorithm (after "Study.com"
 *   (n.d.) and "GeeksForGeeks") to identify the indices of
 *   a pattern in a text corpus.
 *   
 * Here, I am using a list of the fifty territories in the
 *   United States of America as my text corpus (Wikipedia
 *   c.a. Mar. 11, 2025), and saved the text file into a 
 *   file called "us_states.txt". By default, this utility
 *   will load "us_states.txt" as its text corpus. But if an
 *   argument is supplied in the command line, it will load
 *   that file as a text corpus instead.
 *   
 * Program.java is just the driver code to make use of the BMSearch
 *   class, which is located in BMSearch.java. That is where the
 *   main logic of the Boyer-Moore search is located.
 *   
 *
...
public class Program {
	 ...
	 *                the first argument loads a text corpus. If 
	 *                the first argument is left blank, by default,
	 *                it will load "us_states.txt" ...
	 */
	public static void main(String[] args) {
		...
		try {
			// Print out progress ...
			System.out.println("[INFO] Loading text corpus ...");
			String usStatesList;
			if (args.length > 1)
				usStatesList = loadList(args[1]);
			else
				usStatesList = loadList("us_states.txt");
			System.out.println("[INFO] Initalizing the Boyer-Moore Search ...");
			int SIZEOF_TEXT = usStatesList.toCharArray().length;
			BMSearch search = new BMSearch(SIZEOF_TEXT);
			...
			while(!done) {
				...
				case 2: // The Boyre-Moore Search
					System.out.print("Input a pattern to search for: ");
					String thisPattern = inputMechanism.next();
					search.BoyerMooreIndex(usStatesList, thisPattern);
					break;
	 ...
	 *                list to load.
	 * @returns a String of a list of lines in a text file.
	 */
	...
	private static void welcome() {
		System.out.println("\n===================================================");
		System.out.println("=      Boyer-Moore Pattern Index Calculator       =");
